...
function Dashboard() {
    ...
    return (
        <div className="dashboard-container flex h-screen">
            ...
            {/* SideNav */}
            <div className={`fixed top-0 left-0 h-screen w-64 text-white shadow-lg z-50 transform ${sideNavOpen ? "translate-x-0" : "-translate-x-full"} transition-transform duration-300 ease-in-out sm:translate-x-0 sm:block`}>
                <button onClick={() => setSideNavOpen(false)} className="absolute top-4 right-4 text-white sm:hidden">
                    <X size={24} />
                </button>
                <SideNav setActiveComponent={setActiveComponent} />
            </div>

            {/* Overlay for SideNav on small screens */}
            {sideNavOpen && <div className="fixed inset-0 bg-black opacity-50 z-40 sm:hidden" onClick={() => setSideNavOpen(false)}></div>}
